...
#define APP_TICKS_READ_INPUTS 10
...
{
    //APP_LOG_INFO("Start application main loop\n");
    if (app == NULL)
    {
        return -1;
    }
...
static void app_cyclic_data_callback(app_subslot_t* subslot, void* tag)
{
	...
	uint8_t outdata_buf[APP_GSDML_OUTPUT_MAX_SIZE]; /* Todo: Remove temporary buffer */
	...
	if (subslot->data_cfg.insize > 0)
	{
		 ...
		 * For the application data includes
...
{
	APP_LOG_INFO("Init P-Net stack and application\n");
...
/**
 * Read&update input states
 *
 * Actual reading is done every APP_TICKS_READ_INPUTS invocation
 *
 * @param app             InOut:    Application handle
 */
static void update_input_states(app_data_t* app)
{
	app->inputs_tick_counter++;
	if (app->inputs_tick_counter > APP_TICKS_READ_INPUTS)
	{
		app->inputs_tick_counter = 0;
	}
}
